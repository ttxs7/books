...
\begin{document}\begin{tikzpicture}\begin{axis}[scaled ticks=false,view={115}{30},axis lines = none,xlabel=$x$,ylabel=$y$,zlabel=$z$,colormap={gray}{color=(gray) color=(gray)},z buffer=sort,shader=interp,fill opacity=.2,text opacity=1]
    ...
    \draw(0,0,1)node{$\bullet$}node[above]{$N$}--(intersection-1)node{$\bullet$}node[left]{$(X,Y,Z)$}--(1.5,-1.5,0)node{$\bullet$}node[above]{$z=x+iy$};
